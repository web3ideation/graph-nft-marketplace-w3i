...
export class EventEmitted extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save EventEmitted entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type EventEmitted must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("EventEmitted", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): EventEmitted | null {
    return changetype<EventEmitted | null>(
      store.get("EventEmitted", id.toHexString())
    );
  }
